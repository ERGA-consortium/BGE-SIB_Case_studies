...
#while read ID; do
    echo "Doing variant calling for ${ID} MT..."
    gatk --java-options "-Xms70G -Xmx70G" HaplotypeCaller \
	 -ERC GVCF \
	 -I $out2/${ID}.bqsr.bam \
	 -O $vcfdir/${ID}.MT.g.vcf \
	 -ploidy 1 \
	 -L NC_004028.1 \
	 -R $ref1 2> $logdir/${ID}.MT.g.vcf.log
    echo "End work on $ID"
done < samples.list

while read ID; do
    echo "Doing variant calling for ${ID} chr Y..."
    gatk --java-options "-Xms70G -Xmx70G" HaplotypeCaller \
	 -ERC GVCF \
	 -I $out2/${ID}.bqsr.bam \
	 -O $vcfdir/${ID}.Y.g.vcf \
	 -ploidy 1 \
	 -L NC_084851.1 \
	 -R $ref1 2> $logdir/${ID}.Y.g.vcf.log
    echo "End work on $ID"
done < male.miss.list
