...
parser = argparse.ArgumentParser(description="script to remove PCR duplicates from paired end fastq files")
...
#parser.add_argument("-i", "--indices", help="oligos used to identify PCR duplicates in fastq format(can be zipped)",type=str, required=True)
...
#if args.indices.endswith(".gz"):
#	oligoReader=gzip.open(args.indices)
#else:
#	oligoReader=open(args.indices)


r1out=args.read1.split("/")[-1].split(".fastq")[0]+"_PCRfiltered.fastq" # CHECK IF THERE IS NO OTHER DOT PLUS FASTQ IN FILE NAME
r2out=args.read2.split("/")[-1].split(".fastq")[0]+"_PCRfiltered.fastq"
...
#seqCount=0
#dupCount=0
#for oline in oligoReader:
#	lineCount+=1
#
#	if lineCount==1:
#		rID=oline.strip().split(" ")[0]
#		#print(rID)
#
#	elif lineCount==2:
#		curOligo=oline.strip()
#		readToOligo[rID]=curOligo
#
#	elif lineCount==4:
#		lineCount=0
#		seqCount+=1
...
statWriter=open(args.out+"/"+args.read1.split("/")[-1].split(".fastq")[0][:-2]+"_filterPCRdups_Stats.tsv", "w")


r1ClustWriter=open(args.out+"/"+args.read1.split("/")[-1].split(".fastq")[0]+"_PCRfiltered_CLUSTERS.fastq", "w")
r2ClustWriter=open(args.out+"/"+args.read2.split("/")[-1].split(".fastq")[0]+"_PCRfiltered_CLUSTERS.fastq", "w")
...
statWriter.write("######################################################\n")
for key in sorted(iter(countDICT.keys())):
   # print "%s: %s" % (key, countDICT[key])
    statWriter.write("%s\t%s\n"%(key, countDICT[key]))
statWriter.write("######################################################\n")
for key, value in sorted(iter(oligoDICT.items()), key=lambda k,v: (v,k), reverse=True):
    #print "%s: %s" % (key, value)
    statWriter.write("%s\t%s\n"%(key, value))
