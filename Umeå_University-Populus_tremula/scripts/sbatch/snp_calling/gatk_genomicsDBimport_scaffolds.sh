...
#SBATCH --output=reports/sbatch/genomicDB_scaffolds/sbatch_R-%x_%j.out
#SBATCH --error=reports/sbatch/genomicDB_scaffolds/sbatch_R-%x_%j.err
...
## genomicsDBimport
echo "gatk --java-options "-Xmx120g -DGATK_STACKTRACE_ON_USER_EXCEPTION=true" GenomicsDBImport --genomicsdb-workspace-path ${output_dir} --sample-name-map mimmi/aspen_snp_call/info_files/cohort_files/scaffolds.txt --batch-size 20 --merge-contigs-into-num-partitions 25 --bypass-feature-reader -L info_files/scaffolds.bed"
